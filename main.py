...
parser.add_argument(
    "--challenge_data", type=str, default=None
)  # challenge data in test_sample -> input
...
def run_epoch(train, rollout, hp):
    ...
    for batch_idx, (data) in enumerate(tqdm(loader, desc="Processing batches")):
        ...
        if train == 1:
            ...
            _, batch_losses, batch_losses2 = model.sample(
                data, rollout=True, burn_in=hp["burn_in"]
            )  # , TEST=TEST)
...
def run_sanity(args, test_loader):
    ...
    print(
        "(endpoint):"
        + " $"
        + "{:.2f}".format(avgL2_m["e_e_p"])
        + " \pm "
        + "{:.2f}".format(avgL2_sd["e_e_p"])
        + "$ &"
        + " $"
        + "{:.2f}".format(avgL2_m["e_e_v"])
        + " \pm "
        + "{:.2f}".format(avgL2_sd["e_e_v"])
        + "$ &"
    )
...
def create_metadata(file_paths, total_time_steps, overlap, output_path):
    ...
    Args:
        ...
        total_time_steps (int): Total number of time steps in each chunk.
        overlap (int): Number of overlapping time steps between consecutive chunks.
    ...
    for file_path in file_paths:
        ...
        for i in range(1, len(play_on_indices)):
            if play_on_indices[i] != play_on_indices[i - 1] + 1:
                ...
                if (
                    end_idx - start_idx + 1 >= total_time_steps
                ):  # Check total length condition
                    for j in range(
                        start_idx,
                        end_idx - total_time_steps + 1,
                        total_time_steps - overlap,
                    ):
                        metadata.append(
                            {
                                "file_path": file_path,
...
def split_metadata_by_date(
    metadata, val_ratio=0.1, test_ratio=0.1, val_games=None, test_games=None
):
    """
    ...
    if val_ratio is not None and test_ratio is not None:
        train_files, temp_files = train_test_split(
            files, test_size=val_ratio + test_ratio, shuffle=False
        )
        ...
        train_files, temp_files = train_test_split(
            files, test_size=(val_games + test_games) / len(files), shuffle=False
        )
        val_files, test_files = train_test_split(
            temp_files,
            test_size=test_games / (val_games + test_games),
            shuffle=False,
        )
...
if __name__ == "__main__":
    ...
    file_paths = [
        os.path.join(args.data_dir + "/tracking", file)
        for file in os.listdir(args.data_dir + "/tracking")
    ]
    ...
    if not os.path.exists("./metadata/metadata.pkl"):
        ...
        metadata = create_metadata(
            ...
            args.totalTimeSteps,
            args.overlap,
            output_path="./metadata/metadata.pkl",
        )
    else:
    ...
    print(
        "train: "
        + str(len(train_metadata))
        + " val:"
        + str(len(val_metadata))
        + " test: "
        + str(len(test_metadata))
    )
    ...
    args.agents = (
        [f"l{i}" for i in range(1, 12)] + [f"r{i}" for i in range(1, 12)] + ["b"]
    )
    ...
    if not TEST:
        train_loader = DataLoader(
            Dataset(args, train_metadata),
            batch_size=args.batchsize,
            shuffle=True,
            **kwargs,
        )
        val_loader = DataLoader(
            Dataset(args, val_metadata),
            batch_size=args.batchsize,
            shuffle=False,
            **kwargs,
        )
    ...
    test_loader = DataLoader(
        Dataset(args, test_metadata, challenge_data=challenge_data),
        batch_size=args.batchsize,
        shuffle=False,
        **kwargs,
    )
    ...
    if True:
        ...
        for batch_idx, (data) in enumerate(loader):
            ...
            sample, output, output2 = model.sample(
                data,
                rollout=True,
                burn_in=args.burn_in,
                n_sample=1,
        ...
        if (
            ...
            for seq in range(samples.shape[2]):
                sample_ = samples[args.burn_in :, 0, seq].reshape((-1, 23, n_feat))[
                    :, :, :2
                ]  #
                ...
                df.columns = [
                    ...
                    .replace("agent_18", "r8")
                    .replace("agent_19", "r9")
                    .replace("agent_20", "r10")
                    .replace("agent_21", "r11")
                    for col in df.columns
                ]
                df.columns = [
                    col.replace("agent_22", "b")
                    .replace("agent_1", "l2")
                    .replace("agent_2", "l3")
                    for col in df.columns
                ]
